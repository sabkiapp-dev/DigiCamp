...
def fetch_machine_status(user_id, host_forcefully, port_forcefully, ussd_forcefully, sms=False):
    # Fetch the hosts for the user
    if(not sms):
        user_hosts = UserHosts.objects.filter(user_id=user_id, status=1, host=host_forcefully)
    else:
        user_hosts = UserHosts.objects.filter(user_id=user_id, status=1, allow_sms=1)
        ...
        try:
            ...
            if ussd_forcefully == "1" and host_forcefully == host:
                ...
                response = requests.get(
                    f'https://{host}.sabkiapp.com/api/status/ports',
                    params={'port_forcefully': port_forcefully, 'ussd_forcefully': ussd_forcefully},
                    headers=headers,
                    timeout=10
                )
            else:
